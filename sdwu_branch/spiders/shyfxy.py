...
class WYXYSpider(Spider):
    name = 'shyfxy'
    start_urls = ['https://shyfxy.sdwu.edu.cn/index/xyxw/{}.htm'.format(str(i)) for i in range(46, 1, -1)]
    start_urls.insert(0,'https://shyfxy.sdwu.edu.cn/index/xyxw.htm')
    def parse(self, response):
        ...
        for one_selector in selector_list:
            ...
            if'https' in content_url:
                ...
                content_url = 'http://shyfxy.sdwu.edu.cn/'+content_url
                yield scrapy.Request(content_url, meta={"item": item},callback=self.parse_content)



    def parse_content(self,response):
        content = response.xpath("//div[contains(@id,'vsb_content4') or contains(@id,'vsb_content')]//text()").extract()
