...
class WYXYSpider(Spider):
    name = 'jjxy'
    start_urls = ['https://jjxy.sdwu.edu.cn/xwxx/{}.htm'.format(str(i)) for i in range(11, 1, -1)]
    start_urls.insert(0,'https://jjxy.sdwu.edu.cn/xwxx.htm')
    def parse(self, response):
        selector_list = response.xpath("//div[@class='main_conRCb']/ul/li")
        for one_selector in selector_list:
            item = SdwuBranchItem()
            item['title']=one_selector.xpath("a/em/text()").extract()
            item['times']=one_selector.xpath("a/span/text()").extract()
            content_url = one_selector.xpath('a/@href').extract()[0].strip('../')
            if'https' in content_url:
                continue
            else:
                content_url = 'http://jjxy.sdwu.edu.cn/'+content_url
