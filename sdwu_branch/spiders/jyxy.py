...
class WYXYSpider(Spider):
    name = 'jyxy'
    start_urls = ['https://jyxy.sdwu.edu.cn/index/xyxw/{}.htm'.format(str(i)) for i in range(51, 1, -1)]
    start_urls.insert(0,'https://jyxy.sdwu.edu.cn/index/xyxw.htm')
    def parse(self, response):
        selector_list = response.xpath("//div[@class='list111 list222']/ul/li")
        for one_selector in selector_list:
            ...
            if'https' in content_url:
                ...
                content_url = 'http://jyxy.sdwu.edu.cn/'+content_url
                yield scrapy.Request(content_url, meta={"item": item},callback=self.parse_content)



    def parse_content(self,response):
        content = response.xpath("//div[@id='vsb_content']//text()")
